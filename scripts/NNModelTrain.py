...
def main() -> None:
   ...
   # reads in simulated dataset that will be used to fit our NN model:
   ...
   if valdatafile is not None:
       ...
       testsim_x, testsim_y, testcolumnnames, testuniquegenos = nngenotype.read_data_file(valdatafile, shuffle=False)

   # report where output will go:
   outputbase = args.outputfilebase
   print("Will write model to " + outputbase + ".nn.model, model accuracy stats to " + outputbase + ".nn.stats")

   trainsim_x, trainsim_y, traincolumnnames, trainuniquegenos = nngenotype.read_data_file(simdatafile)

   # now optimize parameters using GridSearchCV (saving GSCV's best estimator model fit on all the data with the best params)
   grid_sim = nngenotype.nngrid(trainsim_x, trainsim_y)
   best_params_sim = grid_sim.best_params_  # finds best parameters
   nn_model =  grid_sim.best_estimator_  # model fit with all data and best hyperparameters
   best_params_score = grid_sim.best_score_ # mean CV score of model fit with all data and best hyperparameters

   # save the optimized model to <outputbase>.nn.model:
   nngenotype.save_model_to_file(nn_model, outputbase + ".nn.model")

   statsfile = outputbase + ".nn.stats"
   with open(statsfile, 'w') as statswriter:
      #statswriter.write("NN model best hyperparameters:\nBest number of estimators: " + str(best_params_sim['n_estimators']) + "\nBest maximum depth: " + str(best_params_sim['max_depth']) + "\n")
      statswriter.write("NN model mean CV score (all data, best hyperparameters):\n" + str(best_params_score) + "\n")

      # testing simulated model with validation samples, if they were provided
      if testsim_x is not None:
         test_preds = nn_model.predict(testsim_x)  # predicting x variables from validation set using model
         test_actual = testsim_y
         print("Accuracy of fitted NN model for held out validation set:")
         test_accuracy = nngenotype.accuracy_score(test_actual, test_preds)
