...
def main() -> None:
   ...
   simfeatures, simgenotypes, columnnames, uniquegenos = mlgenotype.read_data_file(simdatafile)
   trainsim_x, testsim_x, trainsim_y, testsim_y = mlgenotype.separate_test_data(simfeatures, simgenotypes)
   ...
   cal_score_output_sim = mlgenotype.best_rf_model_and_score(trainsim_x.to_numpy(), trainsim_y.to_numpy(), best_params_sim['n_estimators'], best_params_sim['max_depth'])
   ...
   print(mlgenotype.accuracy_score(test_actual, test_preds))
   ...
   if realdatafile != '':
      realfeatures, realgenos, realcolumns, realuniquegenos = mlgenotype.read_data_file(realdatafile, shuffle=False)
